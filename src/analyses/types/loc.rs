...
impl<'a> Display for TypeLoc<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            TypeLoc::FunctionTypeParam { index } => write!(f, "type parameter {}", index),
...
impl<'a> TypeLoc<'a> {
    pub fn as_ptr(&self) -> TypeLocPtr {
        match *self {
            ...
            TypeLoc::FunctionTypeParam { index } => TypeLocPtr::FunctionTypeParam { index },
...
impl TypeLocPtr {
    ...
    pub unsafe fn as_ref<'a>(&self) -> TypeLoc<'a> {
        match *self {
            ...
            TypeLocPtr::FunctionTypeParam { index } => TypeLoc::FunctionTypeParam { index },
