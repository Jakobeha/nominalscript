macro_rules! impl_by_map {
    (<$a:lifetime, $Lhs:ident : $Bound1:path | $Bound1a:path, $Rhs:ident : $Bound2:path | $Bound2a:path> $Trait:ident<$a2:lifetime> $tt:tt for $($Functor:ident),+) => {
$(
impl_by_map!(<$a, $Lhs: $Bound1 | $Bound1a, $Rhs: $Bound2 | $Bound2a> $Trait<$a2> $tt for only $Functor);
)+
    };
    (<$a:lifetime, $Lhs:ident : $Bound1:path, $Rhs:ident : $Bound2:path | $Bound2a:path> $Trait:ident<$a2:lifetime> $tt:tt for $($Functor:ident),+) => {
$(
impl_by_map!(<$a, $Lhs: $Bound1, $Rhs: $Bound2 | $Bound2a> $Trait<$a2> $tt for only $Functor);
)+
    };
    (<$a:lifetime, $Lhs:ident : $Bound1:path, $Rhs:ident : $Bound2:path> $Trait:ident<$a2:lifetime> $tt:tt for $($Functor:ident),+) => {
$(
impl_by_map!(<$a, $Lhs: $Bound1, $Rhs: $Bound2> $Trait<$a2> $tt for only $Functor);
)+
    };
    (<$a:lifetime, $Bound1:path, $Bound2:path> $Trait:ident<$a2:lifetime> $tt:tt for $($Functor:ident),+) => {
$(
impl_by_map!(<$a, Lhs: $Bound1, Rhs: $Bound2> $Trait<$a2> $tt for only $Functor);
)+
    };
    (<$a:lifetime, $Bound:path> $Trait:ident<$a2:lifetime> $tt:tt for $($Functor:ident),+) => {
$(
impl_by_map!(<$a, Lhs: $Bound, Rhs: $Bound> $Trait<$a2> $tt for only $Functor);
)+
    };
    (<$a:lifetime> $Trait:ident<$a2:lifetime> $tt:tt for $($Functor:ident),+) => {
$(
impl_by_map!(<$a, Lhs, Rhs> $Trait<$a2> $tt for only $Functor);
)+
    };
...
impl_by_map!(<Lhs, Rhs> $Trait $tt for only $Functor);
    ...
    (<$a:lifetime, $Lhs:ident $(: $Bound1:path $(| $Bound1s:path)*)?, $Rhs:ident $(: $Bound2:path $(| $Bound2s:path)*)?> $Trait:ident<$a2:lifetime> (fn $method:ident($($tt:tt)*) $(with $clone:ident)?) for only $Functor:ident) => {
impl_by_map!(<$a, $Lhs $(0 $(+ $Bound1s)*: $Bound1)?, $Rhs $(: $Bound2 $(+ $Bound2s)*)?> $Trait<$a2> (fn $method($($tt)*) $(with $clone)? by map) for only $Functor);
    };
    (<$a:lifetime, $Lhs:ident $(: $Bound1:path $(| $Bound1s:path)*)?, $Rhs:ident $(: $Bound2:path $(| $Bound2s:path)*)?> $Trait:ident<$a2:lifetime> (fn $method:ident(self $(, $arg:ident: $arg_ty:ty)*) $(with $clone:ident)? by $map:ident) for only $Functor:ident) => {
impl<$a, $Lhs $(: $Bound1 $(+ $Bound1s)*)?, $Rhs: $Trait<$a, $Lhs> $(+ $Bound2 $(+ $Bound2s)*)?> $Trait<$a2, $Functor<$a, $Lhs>> for $Functor<$a, $Rhs> {
...
impl<$a, $Lhs $(: $Bound1 $(+ $Bound1s)*)?, $Rhs: $Trait<$a, $Lhs> $(+ $Bound2 $(+ $Bound2s)*)?> $Trait<$a2, $Functor<$a, $Lhs>> for $Functor<$a, $Rhs> {
    ...
    (<$a:lifetime, $Lhs:ident $(: $Bound1:path $(| $Bound1s:path)*)?, $Rhs:ident $(: $Bound2:path $(| $Bound2s:path)*)?> $Trait:ident<$a2:lifetime> (type $Assoc:ident) for only $Functor:ident) => {
impl<$a, $Lhs $(: $Bound1 $(+ $Bound1s)*)?, $Rhs: $Trait<$a, $Assoc = $Lhs> $(+ $Bound2 $(+ $Bound2s)*)?> $Trait<$a2> for $Functor<$a, $Rhs> {
...
impl<$a, $Lhs $(: $Bound1 $(+ $Bound1s)*)?, $Rhs: $Trait<$a, $Lhs> $(+ $Bound2 $(+ $Bound2s)*)?> $Trait<$Functor<$a, $Lhs>> for $Functor<$a, $Rhs> {
    fn $method(self $(, $arg: $arg_ty)*) -> $Functor<$a, $Lhs> {
        self $(.$clone())? .$map::<$Lhs>(|rhs| rhs.$method($($arg),*))
    }
}
